...
export async function PATCH(
  ...
  try {
    ...
    // 1. Buscar matrícula com todos os dados necessários
    ...
    if (acao === "APROVAR") {
      ...
      // 3. Buscar uma turma disponível para o curso
      ...
      // 4. Atualizar status da matrícula para ATIVA
      await prisma.matricula.update({
        where: { id: matriculaId },
        data: { statusId: statusAtiva.id }
      });

      // 5. CRIAR O ALUNO (Aqui está a mágica! ✨)
      const aluno = await prisma.aluno.create({
        data: {
          matriculaId: matriculaId,
          turmaId: turma.id,
          userId: null // Opcional: se tiver login do usuário
        },
        include: {
          turma: {
            include: {
      ...
      // 6. Atualizar vagas disponíveis da turma
      await prisma.turma.update({
        where: { id: turma.id },
        data: { vagasDisponiveis: turma.vagasDisponiveis - 1 }
      });

      // 7. Atualizar status do pagamento para APROVADO (se existir)
      ...
      // 3. Atualizar matrícula para CANCELADA
      await prisma.matricula.update({
        where: { id: matriculaId },
        data: { statusId: statusCancelada.id }
      });

      // 4. Atualizar status do pagamento para REJEITADO
      if (matricula.pagamento) {
